...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i <= num_rolls:
        ...
        if tmp == 1:
            isOne = True
        # print("dic result",i, tmp)
        total += tmp
        i += 1
    if isOne:
        return 1
    # print("dice final", total)
    return total
    # END PROBLEM 1

# # if one dice is 1, then stop and output 1; otherwise return sum
# def roll_dice(num_rolls, dice = six_sided):
#     assert type(num_rolls) == int, 'num_rolls must be an integer.'
#     assert num_rolls > 0, 'Must roll at least once.'
...
# # if one dice is 1, then keep rolling to num_rolls and output 1; otherwise return sum
# def roll_dice(num_rolls, dice = six_sided):
#     assert type(num_rolls) == int, 'num_rolls must be an integer.'
#     assert num_rolls > 0, 'Must roll at least once.'
...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    opp_score = opponent_score // 10 % 10
    player_score = player_score % 10
    if opp_score - player_score == 0:
        return 1
    else:
        return abs(opp_score - player_score) *3
    # END PROBLEM 2
...
def always_roll(n):
    ...
    assert n >= 0 and n <= 10
    # BEGIN PROBLEM 6
    def roll_dice(player_score, opponent_score):
        return n
    return roll_dice
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while player_score < goal:
        ...
        while opp_score < goal:
            if strategy(player_score, opp_score) != base:
                return False
            opp_score += 1
        player_score += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    comp_val = 0
    while i <= 10:
        comp_val = make_averaged(roll_dice, samples_count)(i, dice)
        if comp_val > max:
            max = comp_val
            max_i = i
        i += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    sus_score = sus_points(score + boar_brawl(score, opponent_score)) 
