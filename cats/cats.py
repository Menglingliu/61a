    ...
    if typed == '' and source == '':
        ...
        for i in range(length):
            if typed_words[i] == source_words[i]:
                count += 1
        rate = 100.0 * count / len(typed_words) 
        return rate
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    word = typed_word
    for i in word_list:
        distance = diff_function(typed_word, i, limit)
        if distance <= limit and distance < ans:
            ans = distance
            word = i
    return word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    diff = typed[0] != source[0] # diff = 1 if typed and source have same initial letter; = 0 if not same
    ...
    # Recursion
    # length = min(len(typed), len(source))
    # def autocorrect(index, count):
    #     if index >= length:
    #         return count
    #     if count > limit:
    #         return count
    #     elif typed[index] == source[index]:
    #         return autocorrect(index + 1, count)
    #     return autocorrect(index + 1, count + 1)
    # return autocorrect(0, abs(len(typed) - len(source)))        
...
def compare(typed, source):
    def search(i,j):
        ...
        if j == len(source):
            return len(typed) - i
        if typed[i] == source[j]:
            return min(search(i+1, j+1), search(i, j+1)+1, search(i+1, j)+1)
        return min(search(i+1, j+1)+1, search(i, j+1)+1, search(i+1, j)+1)
    return search(0, 0)
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    progress_rate = count / len(source)
    d = {'id': user_id, 'progress': progress_rate}
    upload(d)
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i) < min_ans:
                min_ans = time(match, j, i)
                min_player_index = j
        player_min_time.append(min_player_index)
    
    for i in player_indices:
        fastest_words_list.append([])
        for j in range(len(player_min_time)):
            if i == player_min_time[j]:
                fastest_words_list[i].append(get_word(match, j))
