def deep_map(f, s):
    ...
    "*** YOUR CODE HERE ***"
    # if type(s) == list:
    #     return [f(s[i]) for i in range(len(s))]
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        torque_left = total_mass(end(left(m))) * length(left(m))
        torque_right = total_mass(end(right(m))) * length(right(m))
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        for b in branches(t):
            highest_value = max(max_path_sum(b), highest_value)
        return highest_value + label(t)


def path_sum(t):
    """Return the sum for all root-to-leaf paths of a tree.
    >>> t = tree(1, [tree(5, [tree(1), tree(3)]), tree(10)])
    >>> max_path_sum(t) # 1, 10
    11
    >>> t2 = tree(5, [tree(4, [tree(1), tree(3)]), tree(2, [tree(10), tree(3)])])
    ...
    if is_leaf(t):
        ...
        ans = []
        for b in branches(t):
            for val in path_sum(b):
                ans.append(val + label(t))
        return ans
# how to write a function that return sums for all branches??
...
# output sum for all routes on tree branches
def path_sum(t):
    if is_leaf(t):
        return [label(t)]
    else:
