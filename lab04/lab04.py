def divide(quotients, divisors):
    ...
    """
    return {k: [x for x in divisors if x % k ==0] for k in quotients}
...
def distance(city_a, city_b):
    ...
    distance = sqrt((a_lat - b_lat)**2 + (a_lon - b_lon)**2)
    return distance
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    new_city = make_city('new_city_name', lat, lon)
    distance_new_a = distance(new_city, city_a)
    distance_new_b = distance(new_city, city_b)
