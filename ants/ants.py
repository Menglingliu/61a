...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b    
            if place.ant and place.ant.is_container and place.ant.can_contain(self):
    ...
    def double(self):
        ...
        if self.doubled == False:
            self.damage *= 2
            self.doubled = True
        if self.is_container and self.ant_contained and self.ant_contained.doubled == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while position.entrance is not None:
            if num_step > self.upper_bound: # upper_bound = 3
                return None
            elif position.is_hive == False:
                if position.bees and num_step >= self.lower_bound : # lower_bound = 5
                    return random_bee(position.bees)
            position = position.entrance
            num_step += 1
        return None

        # bees in the same Place: return random_bee(self.place.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in self.place.bees[:]:
            ...
            if bee.health > 0 and self.health <= amount:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each but have large health value. """
...
# The HungryAnt Class
class HungryAnt(Ant):
    """Eat bee as a whole, and spend 3 turns to chew before it can eat another bee """
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3
    implemented = True
    ...
    def action(self, gamestate):
        ...
        if self.turns_to_chew != 0:
            ...
            if self.place.bees:
                ran_bee = random_bee(self.place.bees)
                ran_bee.reduce_health(ran_bee.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return self.ant_contained is None and other and other.is_container == False
        # END Problem 8a
...
class TankAnt(ContainerAnt):
    """"Tank Ant is a container ant and also deals 1 damage to all bees in its place each turn."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is an thrower ant that is more costly and waterproof"""
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not isinstance(position.entrance, Hive): 
            if position.ant and not isinstance(position.ant, LaserAnt):
                insects_dic[position.ant] = distance
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem Optional 1
        # return self.place.ant is not None or (self.place.ant and self.place.ant.blocks_path)
